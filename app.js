...
const getDataFromApi = (searchTerm, callback)=>{ //gets data from etsy API, passes that data along to the callback (modState)
  const api_key = 'detvzuhzwwcbxd902sabhaf6';
  let searched = searchTerm; //the term the user writes into our input field.
  let category = appState.category; //the category is updated using the updateCategory function, which takes in the value of the selected choice.

    // the etsyURL, a url pieced together using template literals.
  let etsyURL = `https://openapi.etsy.com/v2/listings/active.js?keywords=${searched}%20${category}&limit=16&includes=Images:1&api_key=${api_key}`;
...
const renderState = (state)=>{ //renders the state
  ...
  if (state.pageView===0) { //first checks what it needs to display on the page, depending on what our state looks like. (refer to updateCategory)
    html += `<form class="js-form">
							<h2>I am a </h2>
							<select class="select" id="userCategory" aria-label="select your Etsy filter">
							...
							<h2>and I am a fan of </h2>
							<input type="text" class="js-query" aria-label="Enter your search"><br>
    ...
    appState.results.forEach(function(object){
      html += `<div class="entry">
								...
								<div class="entry-img"><a href="${object.url}" target="_blank" ><img class="img-render" src="${object.image}" alt="Etsy result image"></a></div>
							</div>`;
    });
...
const listeners = ()=>{
  ...
  $('.js-filters').on('submit', '.js-form', (event)=>{ //listens for a submit via the enter key.
